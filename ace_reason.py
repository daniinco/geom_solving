"""
AceReason-Nemotron Evaluator

Evaluates geometry problems using mlx-community/AceReason-Nemotron-1.1-7B-bf16
...
Usage:
    python3 ace_reason.py --dataset data/t_geom_lettered.csv --limit 5
    python3 ace_reason.py --dataset data/t_geom_lettered.csv
"""
...
# Model
# ============================================================================
MODEL_NAME = "mlx-community/AceReason-Nemotron-1.1-7B-bf16"

# ============================================================================
# Generation parameters
# ============================================================================
SOLVE_MAX_TOKENS = 2850  # tokens for the solution step (AceReason generates long CoT)
ANSWER_MAX_TOKENS = 64    # tokens for the answer extraction step

# ============================================================================
# Prompts
# ============================================================================
SYSTEM_PROMPT = """You are an expert geometry problem solver.
Solve the problem carefully and give a final numeric answer."""

ANSWER_SYSTEM_PROMPT = """You are a strict final-answer extraction engine.
Your job is NOT to solve the problem again.
Use only the previous assistant solution and output exactly one short line.
Do not explain. Do not reason. Do not write a new solution."""

SOLVE_INSTRUCTION = "Solve the following geometry problem. Show a complete solution with explanations."

ANSWER_INSTRUCTION = """Extract the final numeric answer from the previous solution.
...
- If the previous solution is inconclusive, make the best numeric guess from it.
- The answer must be an integer or a decimal number."""
...
def extract_number(text: str) -> float | None:
    ...
    # Remove everything inside <think>...</think> if present
    text = re.sub(r'<think>.*?</think>', '', text, flags=re.DOTALL)
    text = text.strip()

    # Try "Ответ: <number>" or "Answer: <number>"
    answer_match = re.search(r'(?:[Оо]твет|[Aa]nswer)[:\s]+(-?\d+[\.,]?\d*)', text)
    if answer_match:
...
def run_evaluation(dataset_path: str, limit: int | None = None):
    ...
    for i, item in enumerate(dataset):
        ...
        solution_clean = solution_clean.replace('<|im_end|>', '').strip()
        ...
        # Force-skip thinking and prefill the required answer prefix.
        # The generated continuation should be only the number.
        prompt_answer += "<think>\n</think>\nAnswer:"
        ...
        answer_clean = ("Answer:" + answer_response).replace('<|im_end|>', '').strip()
    ...
    output_file = output_dir / f"ace_reason_{dataset_name}_{timestamp}.json"
    ...
    solutions_dir = Path("data/ace_reason_solutions")
...
def main():
    parser = argparse.ArgumentParser(
        description="AceReason-Nemotron LLM evaluation for geometry problems",
