...
1. ПОЛНОТА: Перечисли ВСЕ четвёрки концикличных точек из задачи:
   - Четырёхугольник вписан в окружность → его вершины
   - Прямоугольник → всегда вписан, все 4 вершины
   - Равнобедренная трапеция → всегда вписана, все 4 вершины
   - Два прямых угла на один отрезок: AXB=90 и AYB=90 → AXYB (диаметр AB)
   - Две высоты треугольника BH и CK → BKHC
   - Сумма противоположных углов = 180° → четвёрка вписана

2. ФОРМАТ: Каждая четвёрка — ровно 4 буквы слитно (ABCD, не A,B,C,D).

3. ЛОЖНЫЕ ЧЕТВЁРКИ — проверь, нет ли в найденных:
   - Произвольный параллелограмм или ромб (НЕ вписаны в окружность)
   - Произвольная трапеция (только равнобедренная вписана)
   - Центр окружности + точки на окружности (центр НЕ на окружности)
   - Середина диаметра = центр, НЕ на окружности
   - Только 3 точки на окружности без явной четвёртой

4. ПРОПУСКИ: Сравни свой список из п.1 с найденными четвёрками — что пропущено?

5. ИТОГ: Совпадает ли найденное с тем, что должно быть?

ТИПИЧНЫЕ ОШИБКИ:
- Добавлены вершины ромба или параллелограмма как концикличные
- Центр окружности включён в четвёрку вместе с точками на окружности
- Пропущен прямоугольник или равнобедренная трапеция
- Четвёрка содержит не 4, а 3 или 5 букв
...
На основе рассуждения вынеси вердикт: если найденные четвёрки ПОЛНОСТЬЮ совпадают с тем, что должно быть по правилам (ничего не пропущено, ничего лишнего, формат верен) — ответь "Правильно". Если есть ХОТЬ ОДНО отличие — ответь "Неправильно".
...
Найди концикличные точки заново, строго по правилам из инструкции.
Напиши ТОЛЬКО: Условия: <четвёрки через запятую без пробелов>
Если четвёрок нет — напиши ТОЛЬКО: Условия: -
