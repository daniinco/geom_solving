...
1. ПОЛНОТА: Перечисли ВСЕ условия перп/парал из задачи — явные и неявные:
   - Прямые указания: "перп", "⊥", "парал", "||" → perp / parallel
   - Прямой угол при вершине X в фигуре → две стороны в X перпендикулярны
   - Высота из вершины X на YZ → XH perp YZ
   - Параллелограмм/прямоугольник/ромб/квадрат ABCD → AB parallel CD, BC parallel AD
   - Прямоугольник/квадрат → дополнительно AB perp BC
   - Прямоугольный треугольник ABC с гипотенузой AB → AC perp BC
   - Трапеция ABCD → BC parallel AD (основания)
   - Средняя линия, параллельная стороне → parallel
   - 3D: SA перп плоскости ABC → SA perp AB, SA perp AC

2. ФОРМАТ: "AB perp CD" или "AB parallel CD" (строчные perp/parallel, пробелы вокруг).

3. ЛИШНИЕ УСЛОВИЯ — проверь:
   - Для ромба: перпендикулярность диагоналей добавляется ТОЛЬКО если явно сказано в условии
   - Нет ли угловых условий или длин в списке

4. ПУСТОЙ РЕЗУЛЬТАТ: Если условий нет — должно быть "---" (три тире), а не "-" или пусто.

5. ПРОПУСКИ: Сравни свой список из п.1 с извлечёнными условиями — что пропущено?

6. ИТОГ: Совпадает ли извлечённое с тем, что должно быть?

ТИПИЧНЫЕ ОШИБКИ:
- Пропущена параллельность из параллелограмма (нужны обе пары: AB∥CD и BC∥AD)
- Для ромба добавлена перпендикулярность диагоналей без явного указания в задаче
- Пустой результат записан как "-" вместо "---"
- Пропущена перпендикулярность из прямого угла или высоты
...
На основе рассуждения вынеси вердикт: если извлечённые условия ПОЛНОСТЬЮ совпадают с тем, что должно быть по правилам (ничего не пропущено, ничего лишнего, формат верен, правильный пустой маркер "---") — ответь "Правильно". Если есть ХОТЬ ОДНО отличие — ответь "Неправильно".
...
Извлеки условия заново, строго по правилам из инструкции.
Напиши ТОЛЬКО: Условия: <условия через запятую с пробелами>
Если условий нет — напиши ТОЛЬКО: Условия: ---
