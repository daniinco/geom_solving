...
1. Перечисли все условия на длины из задачи (явные числа + равенства + середины + кратные соотношения)
2. Проверь каждое: правильный формат (AB=CD, AB=5, AB=2CD)?
3. Нет ли лишних условий (углы, перпендикулярность — не про длины)?
4. Не пропущены ли: середины (AM=MB), равные стороны, кратные соотношения ("вдвое больше" → X=2Y)?
...
Извлеки условия на длины правильно. Сразу напиши условия строго по формату (напиши "Условия: " и сразу условия через запятую без пробелов). Если условий на длины нет — выведи только: -. Не рассуждай."""
